...
def load_config() -> dict:
    ...
    env_api_key = os.environ.get('TRANSPAGE_API_KEY') or os.environ.get('OPENAI_API_KEY')
    if env_api_key:
        config['api_key'] = env_api_key

    if not config.get('api_key'):
        print('[FAIL] transpage_config.json 未配置 api_key，且未提供 TRANSPAGE_API_KEY / OPENAI_API_KEY')
        sys.exit(1)
